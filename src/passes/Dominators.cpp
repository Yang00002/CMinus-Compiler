...
void Dominators::create_idom(Function *f) {
    int blockCount = post_order_vec_.size();
...
void Dominators::create_dominance_frontier(Function *f) {
    for (auto &bbase : f->get_basic_blocks()) {
        auto b = &bbase;
        auto &pres = b->get_pre_basic_blocks();
        if (pres.size() > 1) {
            for (auto p : pres) {
...
create_dom_tree_succ_helper(std::map<BasicBlock *, std::list<BasicBlock *>> &m,
                            std::map<BasicBlock *, BBset> &to, BasicBlock *b) {
...
void Dominators::dump_cfg(Function *f) {
    ...
    file_output << digraph;
    file_output.close();
    std::string dot_cmd = "dot -Tpng " + f->get_name() + "_cfg.dot" + " -o " +
                          f->get_name() + "_cfg.png";
