...
class rename_helper {
  public:
    ...
    void run_inner(BasicBlock *bb) {
       // std::cout << bb->get_parent()->get_name() << " " << bb->get_name()
       //           << std::endl;
        ...
        for (auto &ins : bb->get_instructions()) {
           // show_instruction(&ins);
            if (ins.is_alloca()) {
                ...
                if (!ai->get_alloca_type()->is_array_type() &&
                    !ai->get_alloca_type()->is_pointer_type()) {
                    name_stack_map[&ins] = {};
                    remove(&ins);
                ...
                if (name_stack_map[ins.get_operand(0)].size()) {
                    replace_map[&ins] =
