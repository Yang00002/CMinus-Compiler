...
#define _TREE_TO_SHARED(type, target)                                          \
  std::shared_ptr<type>(static_cast<type *>(transform_node_iter(target)))
...
ASTNode *AST::transform_node_iter(syntax_tree_node *n) {
  ...
  */
  if (_STR_EQ(n->name, "program")) {
    ...
    // params 字段填充
    auto nullable_para_list = n->children[3]->children[0];
    if (_STR_EQ(nullable_para_list->name, "void") == false) {
      auto &para_list = nullable_para_list;
      std::stack<syntax_tree_node *> s;
      while (para_list->children_num == 3) {
        s.push(para_list->children[2]);
        para_list = para_list->children[0];
      }
      s.push(para_list->children[0]);
      while (!s.empty()) {
        node->params.push_back(_TREE_TO_SHARED(ASTParam, s.top()));
        s.pop();
      }
    }

    node->compound_stmt = _TREE_TO_SHARED(ASTCompoundStmt, n->children[5]);
    ...
    */
    ...
    while (!s.empty()) {
      node->local_declarations.push_back(
          _TREE_TO_SHARED(ASTVarDeclaration, s.top()));
      s.pop();
    }
    while (statement_list->children_num == 2) {
      s.push(statement_list->children[1]);
      statement_list = statement_list->children[0];
    }
    while (!s.empty()) {
      node->statement_list.push_back(_TREE_TO_SHARED(ASTStatement, s.top()));
      s.pop();
    }
    ...
    */
    ...
    /**/ node->expression = _TREE_TO_SHARED(ASTExpression, n->children[2]);
    node->if_statement = _TREE_TO_SHARED(ASTStatement, n->children[4]);
    if (n->children_num == 7)
      node->else_statement = _TREE_TO_SHARED(ASTStatement, n->children[6]);
    else
    ...
    */
    ...
    if (n->children_num == 3) {
      ...
      node->additive_expression =
          _TREE_TO_SHARED(ASTAdditiveExpression, n->children[0]);
      if (_STR_EQ(n->children[1]->children[0]->name, "+"))
        node->op = AddOp::OP_PLUS;
      else
        node->op = AddOp::OP_MINUS;
      node->term = _TREE_TO_SHARED(ASTTerm, n->children[2]);
    ...
    if (_STR_EQ(name, "expression") || _STR_EQ(name, "var") ||
      ...
      */
      if (_STR_EQ(name, "integer")) {
        ...
        num_node->i_val =
            std::stoi(std::string(n->children[0]->children[0]->name));
      } else if (_STR_EQ(name, "float")) {
        num_node->type = CminusType::TYPE_FLOAT;
        num_node->f_val =
            std::stof(std::string(n->children[0]->children[0]->name));
      } else {
        _AST_NODE_ERROR_
      }
      return num_node;
