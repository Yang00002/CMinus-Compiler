...
class CodeGenHelper {
  ...
  public:
    ...
    RegisterIdentifier getGlobalAddress(string name) {
        RegisterIdentifier ret = registerAllocator.allocate();
        codegen->append_inst("la.local", {ret, name});
        return ret;
    }
};
...
void translate_main(CodeGen *codegen) {
    ...
    auto a = helper.getGlobalAddress("a");
    helper.storeInt(a, 10);
    auto op0 = helper.loadInt(a);
    helper.ret(op0);
    helper.endFunction();
}
