...
class CodeGenHelper {
  ...
  public:
    ...
    void initFunction(std::string funcName, int IntCount = 0,
        ...
        stackSize = 16 + IntCount * 8 + FloatCount * 8;
    ...
    FloatRegisterIdentifier loadFloat(RegisterIdentifier from) {
        FloatRegisterIdentifier ret = floatRegisterAllocator.allocate();
        RegisterIdentifier buf = registerAllocator.allocate();
        codegen->append_inst("st.w", {buf, from, "0"});
        codegen->append_inst("movgr2fr.w", {ret, buf});
        registerAllocator.free(buf);
        return ret;
    }
...
void translate_main(CodeGen *codegen) {
    ...
    helper.initFunction("main", 0, 1);
    auto f = helper.allocate();
    auto op0 = helper.getElementOf(f);
    helper.storeFloat(op0, 0x4048f5c3);
    auto op1 = helper.loadFloat(op0);
    auto op2 = helper.fptosi(op1);
    helper.ret(op2);
    helper.endFunction();
}
